...
namespace BackgroundTest
{
    class Sprite
    {
        ...
        {
            if (!string.IsNullOrEmpty(currentAnimation))
                spriteBatch.Draw(texture, position + Offset, CurrentFrameAnimation.FrameRectangle, Tint, rotation, center - centerOffset, scale, spriteEffect, 0);
            else
                spriteBatch.Draw(texture, position + Offset, null, Tint, rotation, center - centerOffset, scale, spriteEffect, 0);
        }
    }
}
