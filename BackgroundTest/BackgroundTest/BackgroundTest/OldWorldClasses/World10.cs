    ...
    {      
        ...
        private void LoadChunk(int x, int y)
        {
            ...
            if (y * chunkTiles > SeaLevel + SeaLevelMax)
            {
                ...
                if (minerRandom.Next(0, 100) < 3)
                {
                    AddBiominators(x, y, (ushort)(random.Next(3, 5)), true);
                }
            }
            else
            {
        ...
        public void Update(GameTime gameTime)
        {
            ...
            //Console.WriteLine("Biominators: " + biominators.Count);
            //Console.WriteLine("Active Blocks: " + activeBlocks.Count);
            //Console.WriteLine("Biomed: " + chunks[5][5].TilesBiomed);
            ...
            if (biominators.Count != 0)
                UpdateBiominators(gameTime);
                ...
                if (ActiveChunk)
                    if (!miners[i].Moving)
                    {
                        ...
                        if (miners[i].DestroyBlock)
                        {
                            ...
                            GetMinerTile(miners[i].X, miners[i].Y, i);
                            ...
                            //else
                            //{
                            //    DrawCircle(CX, CY, TX, TY, miners[i].Size, 0);
                            //    miners[i].TilesEaten += drawnBlocksChanged;
                            //    chunks[miners[i].SChunkX][miners[i].SChunkY].TilesEaten += (ushort)drawnBlocksChanged;
                            //}
        ...
        private void UpdateBiominators(GameTime gameTime)
        {
            int min = 0;
            int max = biominators.Count;

            if (biominators.Count > 100)
            {
                max = random.Next(100, biominators.Count);
                min = max - 100;
            }

            for (int i = max - 1; i >= min; i--)
            {
                GetTile(biominators[i].X, biominators[i].Y);

                //if (biominators[i].BlockID != 3)
                if (ActiveChunk)
                    if (!biominators[i].Moving)
                    {
                        if (chunks[biominators[i].SChunkX][biominators[i].SChunkY].Biominators == 1)
                            biominators[i].SpawnChance = 80;
                        else
                            biominators[i].SpawnChance = 40;

                        //if (biominators.Count < 100)
                        biominators[i].Update(gameTime);
                        //else
                        //    biominators[i].SpeedFinish();
                        
                        if (biominators[i].ChangeBlock)
                        {
                            GetBiominatorTile(biominators[i].X, biominators[i].Y, i);

                            if (ActiveChunk)
                                if (chunks[CX][CY].tiles[TX][TY].BlockID != 0)
                                {
                                    if (chunks[CX][CY].tiles[TX][TY].BlockID != biominators[i].BlockID && !ActiveBlockExists(CX, CY, TX, TY))
                                    {
                                        ChangeBlock((byte)CX, (byte)CY, (ushort)TX, (ushort)TY, biominators[i].BlockID, true);
                                        biominators[i].TilesBiominated++;
                                        chunks[biominators[i].SChunkX][biominators[i].SChunkY].TilesBiomed++;
                                    }
                                    else
                                    {
                                        if (chunks[CX][CY].tiles[TX][TY].BlockID == biominators[i].BlockID)
                                        {
                                            biominators[i].Spawn = false;
                                            biominators[i].HomeLand++;
                                        }
                                    }
                                }
                                else
                            ...
                            biominators[i].ChangeBlock = false;

                            if (biominators[i].Size > 1)
                            {
                                for (int x = -biominators[i].Size + 1; x <= biominators[i].Size - 1; x++)
                                {
                                    for (int y = -biominators[i].Size + 1; y <= biominators[i].Size - 1; y++)
                                    {
                                        GetTile(biominators[i].X + x, biominators[i].Y + y);
                                        if (ActiveChunk)
                                            if (chunks[CX][CY].tiles[TX][TY].BlockID != 0)
                                            {
                                                if (chunks[CX][CY].tiles[TX][TY].BlockID != biominators[i].BlockID && !ActiveBlockExists(CX, CY, TX, TY))
                        ...
                        if (biominators[i].HomeLand >= 2)
                            biominators[i].Active = false;

                        if (chunks[biominators[i].SChunkX][biominators[i].SChunkY].Biominators == 1)
                            biominators[i].Active = true;

                        if (chunks[biominators[i].SChunkX][biominators[i].SChunkY].TilesBiomed > chunks[biominators[i].SChunkX][biominators[i].SChunkY].BiomableTiles)
                        {
                            biominators[i].Spawn = false;
                            biominators[i].Active = false;
                        }

                        if (biominators[i].Spawn && chunks[biominators[i].SChunkX][biominators[i].SChunkY].Biominators < 1000)
                        {
                            AddBiominators(biominators[i].X, biominators[i].Y, biominators[i].BlockID, biominators[i].SChunkX, biominators[i].SChunkY);
                            biominators[i].Spawn = false;
                        }

                        if (!biominators[i].Active)
                        {
                            chunks[biominators[i].SChunkX][biominators[i].SChunkY].Biominators--;
                            biominators.RemoveAt(i);
                        }
                    }
                    else biominators[i].Update(gameTime);
            }
        }

        private void AddBiominators(int ChunkX, int ChunkY, ushort blockID, bool fullCircle)
        {
            ushort posx = (ushort)(minerRandom.Next(0, chunkTiles) + (ChunkX * chunkTiles));
            ushort posy = (ushort)(minerRandom.Next(0, chunkTiles) + (ChunkY * chunkTiles));

            if (fullCircle)
            {
                for (int i = 0; i < 8; i++)
                {
                    biominators.Add(new Biominator());
                    biominators[biominators.Count - 1].Initialize(
                        posx,
                        posy,
                        random.Next(minerMinSpeed, tileSize * 1000) / 1000f,
                        tileSize,
                        ...
                        ChunkX,
                        ChunkY,
                        minerRandom.Next(1, biominatorMaxSize + 1),
                        i,
                        blockID,
                        minerRandom);

                    chunks[ChunkX][ChunkY].Biominators++;
                }
            }
            else
            {
        ...
        private void AddBiominators(int x, int y, ushort blockID, int sChunkX, int sChunkY)
        {
        ...
        public void Draw(SpriteBatch spriteBatch, int CameraNumber)
        {
            ...
            for (int i = 0; i < biominators.Count; i++)
            {
                if (biominators[i].X > RenderTileX && biominators[i].X < RenderTileX2 && biominators[i].Y > RenderTileY && biominators[i].Y < RenderTileY2)
                {
                    position.X = worldPosition.X + (biominators[i].Position.X) + (minerTexture.Width / 2f) - (tileSize / 2);
                    position.Y = worldPosition.Y + (biominators[i].Position.Y) + (minerTexture.Height / 2f) - (tileSize / 2);

                    spriteBatch.Draw(minerTexture, position, null, Color.Green, 0f, Offset, biominators[i].Size + (biominators[i].Size - 1), SpriteEffects.None, 0f);
                }
            }
        }
    }
