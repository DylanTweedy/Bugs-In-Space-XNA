...
namespace BackgroundTest
{
    class Test
    {
        ...
        {
            //Console.WriteLine(Hugo.Position);
            
            switch ((int)CameraManager.CamerasRead[0].Location.Z)
                {
                    case 1:

                        int cx = BackgroundManager.ChunkX;
                        int cy = BackgroundManager.ChunkY;
                        int cSize = BackgroundManager.ChunkSize;

                        while (Hugo.Position.X >= cSize)
                        {
                            Hugo.Position = new Vector2(Hugo.Position.X - (cSize * 2), Hugo.Position.Y);
                            CameraManager.CamerasRead[0].Location.X++;
                            CameraManager.CamerasRead[0].Focus.X -= (cSize * 2);
                            CameraManager.CamerasRead[0].FocusDestination.X -= (cSize * 2);

                            if (CameraManager.CamerasRead[0].Location.X >= cx)                            
                                CameraManager.CamerasRead[0].Location.X = 0;                            
                        }
                        while (Hugo.Position.X <= -cSize)
                        {
                            Hugo.Position = new Vector2(Hugo.Position.X + (cSize * 2), Hugo.Position.Y);
                            ...
                            if (CameraManager.CamerasRead[0].Location.X < 0)
                                CameraManager.CamerasRead[0].Location.X = cx - 1;
                        }

                        while (Hugo.Position.Y >= cSize)
                        {
                            Hugo.Position = new Vector2(Hugo.Position.X, Hugo.Position.Y - (cSize * 2));
                            CameraManager.CamerasRead[0].Location.Y++;
                            CameraManager.CamerasRead[0].Focus.Y -= (cSize * 2);
                            CameraManager.CamerasRead[0].FocusDestination.Y -= (cSize * 2);

                            if (CameraManager.CamerasRead[0].Location.Y >= cy)                            
                                CameraManager.CamerasRead[0].Location.Y = 0;                            
                        }
                        while (Hugo.Position.Y < -cSize)
                        {
                            Hugo.Position = new Vector2(Hugo.Position.X, Hugo.Position.Y + (cSize * 2));
                            ...
                            if (CameraManager.CamerasRead[0].Location.Y < 0)
                                CameraManager.CamerasRead[0].Location.Y = cy - 1;
                        }
            ...
            if (CameraManager.CamerasRead[0].Location.Z == 2)
            {
                Vector4 pos = WorldManager.Worlds[CurrentWorld].GetTile(Position);
                int boundry = WorldManager.Worlds[CurrentWorld].WorldBoundry(pos.X, pos.Y, pos.Z, pos.W);

                //CameraManager.CamerasRead[0].Focus = Vector2.Zero;
                 
                switch (boundry)
                {
                    case -1:
                        Hugo.Position = new Vector2(Position.X + (((int)WorldVariables.ChunkSize * (int)WorldVariables.TileSize) * (int)WorldManager.Worlds[CurrentWorld].chunkX), Position.Y);

                        CameraManager.CamerasRead[0].Focus = new Vector2(CameraManager.CamerasRead[0].Focus.X +
                            (((int)WorldVariables.ChunkSize * (int)WorldVariables.TileSize) * (int)WorldManager.Worlds[CurrentWorld].chunkX)
                            , CameraManager.CamerasRead[0].Focus.Y);
                        break;
                ...
                //Console.WriteLine("Guesstimated Tile: " + pos);
                //Console.WriteLine("Solid: " + WorldManager.Worlds[CurrentWorld].Solid(pos.X, pos.Y, pos.Z, pos.W));
            }
            ...
            if (currentGamepadState.IsButtonDown(Buttons.LeftShoulder))
                CameraManager.CamerasRead[0].RotationDestination += 0.01f;
            ...
            Hugo.Position += new Vector2((physics.MaxVelocity * currentGamepadState.ThumbSticks.Left.X) / (CameraManager.CamerasRead[0].Zoom * 3f), (physics.MaxVelocity * -currentGamepadState.ThumbSticks.Left.Y) / (CameraManager.CamerasRead[0].Zoom * 3f));

            if (currentGamepadState.Triggers.Right != 0)
                CameraManager.CamerasRead[0].ZoomDestination *= (currentGamepadState.Triggers.Right / 10f) + 1f;

            if (currentGamepadState.Triggers.Left != 0)
                CameraManager.CamerasRead[0].ZoomDestination /= (currentGamepadState.Triggers.Left / 10f) + 1f;
            ...
            WorldVariables.TimeMultiplier = (float)((currentGamepadState.ThumbSticks.Right.X * 150f * mult * mult2) + 1f);

            if (currentGamepadState.ThumbSticks.Right.Y > 0.01f)
                WorldVariables.TimeMultiplier = 1 - currentGamepadState.ThumbSticks.Right.Y;

            if (keyboardState.IsKeyDown(Keys.W))
                Hugo.Position += new Vector2(0, -physics.MaxVelocity / CameraManager.CamerasRead[0].Zoom);
            if (keyboardState.IsKeyDown(Keys.A))
                Hugo.Position += new Vector2(-physics.MaxVelocity / CameraManager.CamerasRead[0].Zoom, 0);
