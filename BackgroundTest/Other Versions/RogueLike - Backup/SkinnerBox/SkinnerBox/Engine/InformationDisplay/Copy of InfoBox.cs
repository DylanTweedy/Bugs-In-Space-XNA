    ...
    class InfoBox2
    {
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            ...
            if (BackgroundColor.A != 0)
                Box.Draw(spriteBatch, Position, BackgroundColor * Alpha, Rotation, (BoxSize * Scale) + (TextPadding * 2f), SpriteEffects.None);

            //if (Active && SelectedItem != -1)
            //{

            //    Box.Draw(spriteBatch, Position - new Vector2(0, (TextScale.Y / Text.Count) - (TextScale.Y / Text.Count)), Color.Red * Alpha, Rotation, new Vector2(BoxSize.X * Scale, (BoxSize.Y * Scale) / Text.Count), SpriteEffects.None);
            //}
            // - new Vector2(0, BoxSize.Y * Scale)) + new Vector2(0, LineSpacing),        
        ...
        List<string> parseText(string text, int width)
        {
            string[] wordArray = text.Split(' ');
            List<string> Lines = new List<string>();
            string line = "";
