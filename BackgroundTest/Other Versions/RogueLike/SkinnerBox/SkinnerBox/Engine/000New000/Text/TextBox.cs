...
namespace SkeletonEngine
{
    ...
    class TextBox
    {
        ...
        public void Draw(Camera camera)
        {
            Vector2 pos;
            if (camera != null)
                pos = SpacePosition.CameraTransform(camera.Position, Position).RoughPosition;
            else
                pos = Position.RoughPosition;

            if (BackgroundColor.A != 0)
                Box.Draw(pos, BackgroundColor * Alpha, Rotation, (TextBoxSize + TextPadding) * Scale, SpriteEffects.None);
            ...
            if (Border != null)
            {
                ...
                Border.DrawBorders(pos, ((TextBoxSize + TextPadding) * Scale) / 2f, Rotation, Alpha);
            ...
            if (Lines.Count != 0)
            {
                for (int i = 0; i < Lines.Count; i++)
                {
                    ...
                    {
                        Box.Draw(pos + (Vector2.Transform(new Vector2(0, Lines[i].LinePosition.Y + (Lines[i].LineSize.Y / 2f)), Matrix.CreateRotationZ(Rotation)) * Scale),
                ...
                for (int i = 0; i < Words.Count; i++)
                    if (Words[i].CurrentSettings.HighlightColor != Color.Transparent)
                        if (Words[i].CurrentSettings.Texture == null)
                            Box.Draw(pos + Vector2.Transform((new Vector2(Words[i].Offset.X, Lines[Words[i].Line].LinePosition.Y) +
                            ...
                            Box.Draw(pos + Vector2.Transform((new Vector2(Words[i].Offset.X - (Words[i].WordSize.X / 2f), Lines[Words[i].Line].LinePosition.Y) +
                                (new Vector2(Words[i].WordSize.X, Lines[Words[i].Line].LineSize.Y) / 2f)) * Scale, Matrix.CreateRotationZ(Rotation)),
                ...
                for (int i = 0; i < Words.Count; i++)
                {
                    ...
                    if (Words[i].CurrentSettings.Texture == null)
                    {
                        GlobalVariables.spriteBatch.DrawString(Font, Words[i].Text,
                            pos + Vector2.Transform((Words[i].Offset * Scale), Matrix.CreateRotationZ(Rotation)),
                        ...
                        Words[i].CurrentSettings.Texture.Draw(pos + Vector2.Transform((Words[i].Offset * Scale), Matrix.CreateRotationZ(Rotation)),
