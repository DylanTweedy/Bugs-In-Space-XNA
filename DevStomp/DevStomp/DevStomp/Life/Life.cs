...
namespace DevStomp
{
    class Life
    {
        ...
        public void Update()
        {
            ...
            switch (CS)
            {
                ...
                case 3:
                    ...
                    {
                        Acceleration.Y -= 75f;
                        Power -= (float)GlobalVariables.FrameTime * UsefulMethods.FindBetween(Math.Abs(Velocity.Y), 20f, 0.5f, 0.5f, 0.001f, false);
            ...
            if (PPower == Power)
            Power += (float)GlobalVariables.FrameTime * 0.15f;
            if (Power > 1f)
                Power = 1f;
            if (Power <= 0f)
            {
            ...
            if (!JetPackReady)
            {
                JetPackTimer += (float)GlobalVariables.FrameTime;
                if (JetPackTimer > 5f)
                {
                    JetPackTimer = 0f;
                    JetPackReady = true;
            ...
            Position += (ActualVelocity * (float)GlobalVariables.FrameTime);
        ...
        private void InforBoxUpdate()
        {
            infoBox.UpdatePosition(Position + new Vector2(64, -512));

            body.SetSkinColor(infoBox.SetColor(body.colors.Skin));
        }
        ...
        private void SetSkeleton()
        {
            ...
            {
                Animation = "Jump";
                AnimationSpeed = UsefulMethods.FindBetween(Velocity.Y, 500f, -500f, 1f, -1f, true);
            }
            else if (Math.Abs(Velocity.X) > MaxVelocity * 0.95f)
            {
                Animation = "Run";
                AnimationSpeed = Math.Abs(Velocity.X) / MaxVelocity;
            }
            else if (Math.Abs(Velocity.X) > 0.5f)
            {
                Animation = "Walk";
                AnimationSpeed = Math.Abs(Velocity.X) / (MaxVelocity / 3);
            }
            else
            {
                Animation = "Idle";
            ...
            body.Update(Position, Rotation, Velocity, Animation, AnimationSpeed);
        }
        ...
        private void Collision()
        {
            ...
            //    //{
            //    //    Vector2 SelectedChunk = (data.Position - WorldManager.worlds.Position) / (WorldManager.worlds.QuadSize * WorldManager.worlds.ChunkSize);
            //    //    SelectedChunk.X = (float)Math.Floor(SelectedChunk.X);
            //    //    SelectedChunk.Y = (float)Math.Floor(SelectedChunk.Y);

            //    //    Vector2 ChunkPosition = ((data.Position - WorldManager.worlds.Position) / WorldManager.worlds.QuadSize) - (SelectedChunk * WorldManager.worlds.ChunkSize);
        ...
        {       
            ...
            //else
            //    r = Vector2.Distance(WorldManager.worlds.Position, Position);
        ...
        public void DrawFront(SpriteBatch spriteBatch)
        {
            ...
            {
                float angle = (float)Math.Atan2(ActualVelocity.X, -ActualVelocity.Y);
                float distance = ActualVelocity.Length();
                distance = UsefulMethods.FindBetween(distance, 1000f, 0f, 5f, 0f, false);

                spriteBatch.Draw(StaticTests.MarkerTriangle, Position - new Vector2(0, 128), null, Color.Red, angle, new Vector2(StaticTests.Marker.Width / 2, StaticTests.Marker.Height), new Vector2(0.5f, distance), SpriteEffects.None, 0f);

                angle = (float)Math.Atan2(Acceleration.X, -Acceleration.Y);
                distance = Acceleration.Length();
                distance = UsefulMethods.FindBetween(distance, 300f, 0f, 5f, 0f, false);

                spriteBatch.Draw(StaticTests.MarkerTriangle, Position - new Vector2(0, 128), null, Color.Blue, angle, new Vector2(StaticTests.Marker.Width / 2, StaticTests.Marker.Height), new Vector2(0.5f, distance), SpriteEffects.None, 0f);
            }
        ...
        public void DrawUI(SpriteBatch spriteBatch)
        {
            ...
            {
                case 0:
                    pos.X = GlobalVariables.WindowSize.X * 0.1f;
                    break;
                case 1:
                    pos.X = GlobalVariables.WindowSize.X * 0.3f;
                    break;
                case 2:
                    pos.X = GlobalVariables.WindowSize.X * 0.6f;
                    break;
                case 3:
                    pos.X = GlobalVariables.WindowSize.X * 0.9f;
                    break;
            }
