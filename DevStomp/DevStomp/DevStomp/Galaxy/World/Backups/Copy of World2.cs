...
//        float RotationalVelocityClouds;
...
//            Rotation = (GlobalVariables.RandomNumber.Next(0, 10000) / 10000f) * ((float)Math.PI * 2f);
//            CloudRotation = (GlobalVariables.RandomNumber.Next(0, 10000) / 10000f) * ((float)Math.PI * 2f);
//            OverlayRotation = (GlobalVariables.RandomNumber.Next(0, 10000) / 10000f) * ((float)Math.PI * 2f);

//            RotationalVelocity = GlobalVariables.RandomNumber.Next(0, 10000) / 100000f;
//            RotationalVelocityClouds = GlobalVariables.RandomNumber.Next(0, 10000) / 100000f;
...
//            Chemistry = ElementTable.BuildTable(GlobalVariables.MassConstant * (long)(Area / (Radius * 2)));
...
//            int size = (int)((Radius / (QuadSize * ChunkSize)) * 2f);
//            if (size > UpperLimit)
//                size = UpperLimit;
//            WorldX = size;
//            WorldY = size;
            
//            if (ObjectMass < 1e+10f)
//                ObjectType = 0;
//            else if (ObjectMass < 1e+11f)
//                ObjectType = 1;
//            else if (ObjectMass < 1e+13f)
//                ObjectType = 2;
//            else
//                ObjectType = 3;
...
//            for (int i = 1; i < Chemistry.Count; i++)            
//                m += Chemistry[i];
            
//            //float r = Radius;
//            Radius = m / GlobalVariables.MassConstant;

//            ObjectRadius = Radius * (1f - CloudPercent);
//            Area = (float)Math.PI * (Radius * Radius);
//            Density = Mass / Area;
...
//                ObjectMass = Mass * (1 - CloudPercent);
//                ObjectRadius = Radius * (1f - CloudPercent);
...
//                if (Chemistry[LightElement] > 2 * mult)
//                {
//                    Chemistry[LightElement] -= 2 * mult;
//                    Chemistry[LightElement + 1] += 1 * mult;
//                    UpdateScale();
//                }
//                else
//                {
//                    Chemistry[LightElement] -= Chemistry[LightElement];
//                    Chemistry[LightElement + 1] += Chemistry[LightElement];
//                    LightElement++;
//                    ChemistryChange = true;
//                }
//            }
...
//                Mass = (float)ActualMass * 10f;
//                ObjectMass = Mass * ((1 - CloudPercent) * (1 - CloudPercent));
...
//                Vector2 CameraPosition = InputManager.GetMousePosition(0) - (P.Position + Offset);
...
//            if (!DebugOptions.DebugActive)
//            {
//                //Velocity += Acceleration * (float)WorldVariables.FrameTime;
//                //P.Position += Velocity * (float)WorldVariables.FrameTime;

//                Rotation += RotationalVelocity * (float)GlobalVariables.FrameTime;
//                CloudRotation += RotationalVelocityClouds * (float)GlobalVariables.FrameTime;
...
//                    OverlayRotation -= ((float)Math.PI * 2f) * (OverlayRotation / ((float)Math.PI * 2f));
//            }

//            #region old

//            //float r = Vector2.Distance(Velocity, Vector2.Zero);

//            //Rotation += r / 1000000000f;
...
//            //    P.PositionX = 1 + WorldID;
//            //    P.PositionY = 1 + WorldID;
//            //    P.Position = Vector2.Zero;

//            //    int dis = 200;

//            //    P.PositionX = WorldVariables.RandomNumber.Next(0, dis) - (dis / 2);
//            //    P.PositionY = WorldVariables.RandomNumber.Next(0, dis) - (dis / 2);
//            //    P.Position = new Vector2(WorldVariables.RandomNumber.Next(0, 2000000), WorldVariables.RandomNumber.Next(0, 2000000)) - new Vector2(1000000f, 1000000f);
                

//            //    Vector2 StarPos = Vector2.Zero;
//            //    P.SetRough();
//            //    Vector2 RoughPos = P.RoughPosition;
                

//            //    float Distance3 = Vector2.Distance(StarPos, RoughPos);
//            //    float Force3 = (float)Math.Sqrt((WorldVariables.GravitationalConstant * (CenterMass + Mass)) / Distance3);
...
//            //    Velocity = UsefulMethods.AngleToVector(angle) * Force3;
...
//            ////Console..WriteLine(Vector2.Distance(Vector2.Zero, Velocity));

//            #endregion

//            #region CameraControl
//            //if (InputManager.KB.IsKeyDown(Microsoft.Xna.Framework.Input.Keys.Right) && InputManager.pKB.IsKeyUp(Microsoft.Xna.Framework.Input.Keys.Right))
//            //    SelectedPlanet++;
//            //else if (InputManager.KB.IsKeyDown(Microsoft.Xna.Framework.Input.Keys.Left) && InputManager.pKB.IsKeyUp(Microsoft.Xna.Framework.Input.Keys.Left))
//            //    SelectedPlanet--;
//            //else if (InputManager.KB.IsKeyDown(Microsoft.Xna.Framework.Input.Keys.Down) && InputManager.pKB.IsKeyUp(Microsoft.Xna.Framework.Input.Keys.Down))
//            //    SelectedPlanet = -1;

//            //if (SelectedPlanet < -1)
//            //    SelectedPlanet = 100;
//            //else if (SelectedPlanet > 100)
//            //    SelectedPlanet = -1;
...
//                    TestChunk[(int)EditChunks[i].X][(int)EditChunks[i].Y].CalculateChunk(graphics, new Vector2(QuadSize * ChunkSize * EditChunks[i].X, QuadSize * ChunkSize * EditChunks[i].Y) + (P.Position + Offset), QuadSize, ChunkSize);
//                }
...
//                CameraManager.Cams[0].SetRotation(CameraManager.Cams[0].R + (0.5f * (float)GlobalVariables.FrameTime));
...
//            {
//                if (CameraManager.Cams[i].Z >= 0.1f)
//                {
...
//                        //SetEdges(X, Y);
//                        TestChunk[X][Y].CalculateChunk(graphics, new Vector2(QuadSize * ChunkSize * X, QuadSize * ChunkSize * Y) + (P.Position + Offset), QuadSize, ChunkSize);
...
//            for (int i = 0; i < CameraManager.Cams.Count; i++)
//            {
//                if (CameraManager.Cams[i].Z > TransitionLevel)
//                {
//                    float Distance = Vector2.Distance(P.RoughPosition, CameraManager.Cams[i].GetRoughPosition());
...
//        public void Draw(SpriteBatch spriteBatch, GraphicsDevice graphics, int camera)
//        {
//            int OffsetX = (int)(P.PositionX - CameraManager.Cams[camera].PositionX);
//            int OffsetY = (int)(P.PositionY - CameraManager.Cams[camera].PositionY);
//            Vector2 DrawOffset = new Vector2(OffsetX * 2000000f, OffsetY * 2000000f);
//            float objectScale = (Radius * 2f) * CameraManager.Cams[camera].Z;
//            float originalObjectScale = objectScale;
//            int size = SM.PlanetBody.Size;
//            Vector2 halfsize = new Vector2(size / 2f, size / 2f);
//            float Scale = (float)(Radius * 2f) / size;
//            float multiplier = 1f;
//            float Alpha = 1f;
...
//            if (CameraManager.Cams[camera].Z >= 0.1f)
//            {
...
//                                TestChunk[X][Y].Draw(spriteBatch, graphics, P.Position + DrawOffset, new Vector2(QuadSize * ChunkSize * X, QuadSize * ChunkSize * Y) + Offset, QuadSize, Rotation);
//                    }
//                }
...
//                    bool Acc = CameraManager.Cams[camera].settings.SpaceView.DrawAcceleration;
//                    bool Vel = CameraManager.Cams[camera].settings.SpaceView.DrawVelocity;
...
//                        if (Acc)
//                        {
//                            spriteBatch.Draw(StaticTests.Arrow, P.Position + DrawOffset, null, ObjectColour * alpha * Alpha, UsefulMethods.VectorToAngle(Acceleration),
//                                    new Vector2(StaticTests.Arrow.Width / 2, StaticTests.Arrow.Height), ArrowSize, SpriteEffects.None, 1f);
//                        }
//                        if (Vel)
//                        {
//                            spriteBatch.Draw(StaticTests.Arrow, P.Position + DrawOffset, null, ObjectColour * alpha * Alpha, UsefulMethods.VectorToAngle(Velocity),
//                                    new Vector2(StaticTests.Arrow.Width / 2, StaticTests.Arrow.Height), ArrowSize, SpriteEffects.None, 1f);
//                        }
//                    }
...
//                    if (CloudPercent != 0f)
//                    {
//                        spriteBatch.Draw(TexDust, P.Position + DrawOffset, SM.StarsBig.Rectangles[IDBody], ColorBody * Alpha * 0.5f, CloudRotation, halfsize / 2f,
//                            (Scale) + (Scale * CloudPercent * 10f), SpriteEffects.None, 0f);

//                        spriteBatch.Draw(TexDust, P.Position + DrawOffset, SM.StarsBig.Rectangles[IDClouds], ColorOverlay * Alpha * 0.5f, Rotation, halfsize / 2f,
...
//                    if (CloudPercent != 1f)
//                    {
//                        spriteBatch.Draw(TexBody, P.Position + DrawOffset, SM.PlanetBody.Rectangles[IDBody], ColorBody * Alpha, Rotation, halfsize, Scale * (1f - CloudPercent), SpriteEffects.None, 0f);

//                        switch (ObjectType)
//                        {
//                            case 0:
//                                spriteBatch.Draw(TexOverlay, P.Position + DrawOffset, SM.PlanetBody.Rectangles[IDOverlay], ColorOverlay * 0.7f * Alpha, Rotation + OverlayRotation, halfsize, Scale * (1f - CloudPercent), SpriteEffects.None, 0f);
//                                spriteBatch.Draw(TexGlow, P.Position + DrawOffset, SM.PlanetBody.Rectangles[IDGlow], ColorOverlay * 0.7f * Alpha, Rotation, halfsize, Scale * (1f - CloudPercent), SpriteEffects.None, 0f);
//                                break;

//                            case 3:
//                                spriteBatch.Draw(TexClouds, P.Position + DrawOffset, SM.PlanetBody.Rectangles[IDClouds], ColorBody * 0.2f * Alpha, -CloudRotation, halfsize, Scale * 2f * (1f - CloudPercent), SpriteEffects.None, 0f);
//                                spriteBatch.Draw(TexClouds, P.Position + DrawOffset, SM.PlanetBody.Rectangles[IDClouds], ColorBody * 0.2f * Alpha, CloudRotation, halfsize, Scale * 2f * (1f - CloudPercent), SpriteEffects.None, 0f);
//                                spriteBatch.Draw(TexBody, P.Position + DrawOffset, SM.PlanetBody.Rectangles[IDBody], Color.White * 0.5f * Alpha, Rotation + OverlayRotation, halfsize, Scale * (1f - CloudPercent), SpriteEffects.None, 0f);
//                                spriteBatch.Draw(TexGlow, P.Position + DrawOffset, SM.PlanetBody.Rectangles[IDGlow], ColorBody * 1f * Alpha, CloudRotation + OverlayRotation, halfsize, Scale * (1f - CloudPercent), SpriteEffects.None, 0f);
//                                spriteBatch.Draw(TexGlow, P.Position + DrawOffset, SM.PlanetBody.Rectangles[IDGlow], Color.White * 0.7f * Alpha, CloudRotation, halfsize, Scale * (1f - CloudPercent), SpriteEffects.None, 0f);
//                                break;

//                            default:
//                                spriteBatch.Draw(TexOverlay, P.Position + DrawOffset, SM.PlanetBody.Rectangles[IDOverlay], ColorOverlay * 0.7f * Alpha, Rotation + OverlayRotation, halfsize, Scale * (1f - CloudPercent), SpriteEffects.None, 0f);
//                                spriteBatch.Draw(TexClouds, P.Position + DrawOffset, SM.PlanetBody.Rectangles[IDClouds], ColorClouds * 0.85f * Alpha, CloudRotation, halfsize, Scale * 1.01f * (1f - CloudPercent), SpriteEffects.None, 0f);
//                                spriteBatch.Draw(TexGlow, P.Position + DrawOffset, SM.PlanetBody.Rectangles[IDGlow], ColorOverlay * 0.8f * Alpha, Rotation, halfsize, Scale * (1f - CloudPercent), SpriteEffects.None, 0f);
//                                float angle = UsefulMethods.VectorToAngle(-P.RoughPosition);
//                                spriteBatch.Draw(TexShadows, P.Position + DrawOffset, SM.PlanetBody.Rectangles[IDShadows], Color.White * Alpha, angle, halfsize, Scale * (1f - CloudPercent), SpriteEffects.None, 0f);
//                                break;
//                        }
//                    }
...
//                    if (Alpha != 1f)
//                        spriteBatch.Draw(TexSmall, P.Position + DrawOffset, SM.StarsBack.Rectangles[IDSmall], ColorBody * (1f - Alpha), Rotation, new Vector2(16, 16), Scale * 128, SpriteEffects.None, 0f);

//                    #region Information Box

//                    if (CameraManager.Cams[camera].settings.SpaceView.ShowInfo)
//                    {
//                        alpha = 1f;
//                        if (objectScale >= 200f)
...
//                            StaticInfoBox.AddItem("Acceleration: " + Vector2.Distance(Vector2.Zero, Acceleration));
//                            StaticInfoBox.AddItem("Velocity: " + Vector2.Distance(Vector2.Zero, Velocity));
//                            StaticInfoBox.AddItem("Mass: " + (float)ObjectMass);
//                            StaticInfoBox.AddItem("Area: " + (float)Area);
//                            StaticInfoBox.AddItem("Scale: " + Scale);
//                            StaticInfoBox.AddItem("Density: " + Density);
//                            StaticInfoBox.AddItem("Cloud Percent: " + CloudPercent);
//                            StaticInfoBox.AddItem(ElementTable.Elements[LightElement].Name + ": " + Chemistry[LightElement]);
//                            StaticInfoBox.AddItem(ElementTable.Elements[LightElement + 1].Name + ": " + Chemistry[LightElement + 1]);
...
//                            StaticInfoBox.Draw(spriteBatch, P.Position + DrawOffset + new Vector2(Radius * 2.5f, -Radius * 1.1f), (Radius / 100f) * multiplier, alpha);
...
//                else
//                {
//                    spriteBatch.Draw(TexSmall, P.Position + DrawOffset, SM.StarsBack.Rectangles[IDSmall], ColorBody, Rotation, new Vector2(16, 16), Scale * 128, SpriteEffects.None, 0f);
//                }
//            }

//            if (DebugOptions.DebugActive)
//            {
//                spriteBatch.Draw(StaticTests.MarkerCircle, P.Position + DrawOffset, null, Color.Yellow * 0.5f, 0f,
