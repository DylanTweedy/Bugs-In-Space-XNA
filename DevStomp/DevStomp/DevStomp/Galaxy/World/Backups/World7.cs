...
namespace DevStomp
{
    class World7
    {
        ...
        public void Initialize(Vector2 position, GraphicsDevice graphics)
        {
            ...
            Position = -new Vector2((WorldX * QuadSize * ChunkSize) / 2, (WorldY * QuadSize * ChunkSize) / 2);
            CenterOfMass = new Vector2(0, (WorldY * QuadSize * ChunkSize) / 2);
            WorldBoundry = (WorldX * QuadSize * ChunkSize) / 2;
            ...
            for (int x = 0; x < TestChunk.Length; x++)
            {
                ...
                for (int y = 0; y < TestChunk[x].Length; y++)
                {
                    ...
                    if (y < 2)
                        ...
                        TestChunk[x][y].InitializeSolid(3);
                    else if (y < 6)
                        TestChunk[x][y].InitializeSolid(2);
                    else
                        TestChunk[x][y].InitializeSolid(1);
                }
            }
            ...
            for (int x = 0; x < TestChunk.Length; x++)
                for (int y = 0; y < TestChunk[x].Length; y++)
                {
                    TestChunk[x][y].CalculateChunk(graphics, new Vector2(QuadSize * ChunkSize * x, QuadSize * ChunkSize * y) + Position, QuadSize, ChunkSize);
                }
        }
        ...
        {      
            ...
            if (CX < 0 || CY < 0 || CX >= WorldX || CY >= WorldY)
            {
            }
            else
            {
        ...
        public CollisionData CheckTile(Vector2 Pos, Vector2 PreviousPos)
        {
            CollisionData data;
            ...
            if (distance <= 1f)
            {
                ...
                if (collisionPoint != Vector2.Zero)
                {
                    if (collisionPoint == Vector2.One)                    
                        return new CollisionData(SolidCollision(Pos), 0f, 0f);
                    
                    Markers.Add(new Marker(collisionPoint, Color.Red, 1f));
                    return new CollisionData(collisionPoint, 0f , 0f);
                }
            }
            else
            {
                ...
                for (int i = 0; i <= distance; i++)
                {
                    ...
                    {
                        if (collisionPoint == Vector2.One)                        
                            return new CollisionData(SolidCollision(PreviousPos + (Dis * i)), 0f, 0f);                        
        ...
        private List<Vector2[]> GetVertices(Vector2 Pos, ChunkPosition CP)
        {
            if (CP == null)
        ...
        private ChunkPosition GetChunkPosition(Vector2 Pos)
        {
            Pos -= Position;
        ...
        public void Update(GraphicsDevice graphics)
        {
            ...
            Vector2 CameraPosition = CameraManager.Cams[0].P + (MousePosition / CameraManager.Cams[0].Z) - Position;
            ...
            if (InputManager.KB.IsKeyDown(Microsoft.Xna.Framework.Input.Keys.F2))
            {
                ...
                TestChunk[0][0].CalculateChunk(graphics, new Vector2(QuadSize * ChunkSize * 0, QuadSize * ChunkSize * 0) + Position, QuadSize, ChunkSize);
            }
            ...
            if (EditedPoints.Count > 0)
            {
                ...
                for (int i = 0; i < EditChunks.Count; i++)
                {
                    ...
                    TestChunk[(int)EditChunks[i].X][(int)EditChunks[i].Y].CalculateChunk(graphics, new Vector2(QuadSize * ChunkSize * EditChunks[i].X, QuadSize * ChunkSize * EditChunks[i].Y) + Position, QuadSize, ChunkSize);
                }
            ...
            for (int x = 0; x < TestChunk.Length; x++)
                for (int y = 0; y < TestChunk[x].Length; y++)
                {
                    //if (TestChunk[x][y] != null)
                        ...
                        //TestChunk[x][y].Draw(spriteBatch, graphics, new Vector2(QuadSize * ChunkSize * x, QuadSize * ChunkSize * y) + Position, QuadSize);
                }
