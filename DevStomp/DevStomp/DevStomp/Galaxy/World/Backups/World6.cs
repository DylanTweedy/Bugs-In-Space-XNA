...
namespace DevStomp
{
    class World6
    {
        ...
        public void Initialize(Vector2 Position, GraphicsDevice graphics)
        {
            ...
            //float before = GC.GetTotalMemory(true);
            ...
            //float after = GC.GetTotalMemory(true);

            //float final = after - before;
        ...
        public Vector2 CheckTile(Vector2 Pos, Vector2 PreviousPos)
        {
            ...
            if (CP != null)
                CollisionChunks.Add(new Vector2((QuadSize * ChunkSize * CP.CX) + (QuadSize * CP.QX) + (QuadSize / 2), (QuadSize * ChunkSize * CP.CY) + (QuadSize * CP.QY) + (QuadSize / 2)));
            ...
            if (Points != null)
                for (int p = 0; p < Points.Count; p++)
                    if (IsPointInPolygon(Pos, Points[p]))
                    {
                        ...
                        Vector2 move = new Vector2((float)Math.Sin(angle), -(float)Math.Cos(angle)) * QuadSize;

                        return Pos - (move * 1.01f);
