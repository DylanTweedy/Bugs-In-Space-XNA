                ...
                if (particles != 0)
                {
                    ...
                    for (int o = 0; o < particles; o++)
                        if (Particles.Count - RemoveParticles < DesiredParticles)
                        {
                            ...
                            Particles[i].Scale = DrawScale + (DrawScale * (((float)rand.NextDouble() * 0.2f) - 0.1f));
        ...
        public void Draw(SpriteBatch spriteBatch, Camera cam, bool front)
        {
            for (int i = 0; i < Particles.Count; i++)
            {
                ...
                if ((front && size > 1f) || (!front && size <= 1f))
                {
                    ...
                    if (Particles[i].Tex.Width > Particles[i].Tex.Height)
                        stretch *= 32f / (float)Particles[i].Tex.Width;
                    else
                        stretch *= 32f / (float)Particles[i].Tex.Height;

                    spriteBatch.Draw(Particles[i].Tex, Particles[i].Position + cam.Position + Particles[i].PositionModifier,
                        null, Particles[i].Tint * Particles[i].Alpha * DrawAlpha,
                        angle, new Vector2(Particles[i].Tex.Width / 2, Particles[i].Tex.Height / 2), stretch * ParticlesPara[i].Depth * Particles[i].Scale,
                        SpriteEffects.None, scale);

                    if (Particles[i].Glow)
                        spriteBatch.Draw(Particles[i].Tex, Particles[i].Position + cam.Position + Particles[i].PositionModifier,
                            null, Color.White * Particles[i].Alpha * DrawAlpha,
                            angle, new Vector2(Particles[i].Tex.Width / 2, Particles[i].Tex.Height / 2), stretch * ParticlesPara[i].Depth * (Particles[i].Scale * 0.5f),
                            SpriteEffects.None, scale);
                }
            }
        }
