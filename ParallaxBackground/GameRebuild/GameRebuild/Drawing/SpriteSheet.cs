...
namespace GameRebuild
{
    ...
    static class SpriteSheet
    {
        ...
        static public Sheet Initialize(ContentManager Content, GraphicsDevice graphicsDevice, string Path, string Name)
        {
            content = Content;
            random = new Random();
            ...
            AddTexture(Path);
            LoadContent(graphicsDevice, Name);
        ...
        static public void LoadContent(GraphicsDevice graphics, string Name)
        {
            ...
            ElementsPerSheet = (size / tileSize) * (size / tileSize);
