...
namespace GameRebuild
{
    static class ColorManager
    {
        ...
        static public List<Color> GetShades(List<Color> rgb)
        {
            List<Color> P = new List<Color>();
            for (int i = 0; i < rgb.Count; i++)
                P.AddRange(ColorManager.GetShades(rgb[i]));

            return P;
        }

        static public List<Color> GetShades(Color rgb)
        {
            List<Color> Palette = new List<Color>();
        ...
        static private Color HSB2RGB(HSB hsb)
        {
            Color rgb = new Color();
            float c = hsb.B * hsb.S;
        ...
        static private HSB RGB2HSB(Color rgb)
        {
            HSB hsb = new HSB();
            float r = rgb.R / 255f;
            float g = rgb.G / 255f;
