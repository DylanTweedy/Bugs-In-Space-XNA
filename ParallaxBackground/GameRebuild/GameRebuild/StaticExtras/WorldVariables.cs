...
namespace GameRebuild
{
    static class WorldVariables
    {
        ...
        static public void SetCameraSize(int CameraNumber, int ChunkSize)
        {
            ...
            ChunksVertical = (int)(Math.Ceiling((WindowHeight / CameraManagerOLD.CamerasRead[CameraNumber].Zoom) / (float)ChunkSize) + 1);
            ChunksHorizontal = (int)(Math.Ceiling((WindowWidth / CameraManagerOLD.CamerasRead[CameraNumber].Zoom) / (float)ChunkSize) + 1);
        }
