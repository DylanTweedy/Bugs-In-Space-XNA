    ...
    public class SkillCloud
    {
        ...
        public List<Light> GetLights(SkillSelector Selector)
        {
                ...
                for (int o = 0; o < Selectors.Count; o++)
                    if (S1.Key == Selectors[o].SelectedSkill)
                    {
                        if (color == Color.White)
                            color = Selectors[o].PlayerProfile.PrimaryColor * 0.5f;
                        else
                            color = Color.Lerp(color, Selectors[o].PlayerProfile.PrimaryColor * 0.5f, 0.5f);
                        ...
                        Selectors[o].light.LightDecay = 200;
                        Selectors[o].light.Power = 2f;
            ...
            foreach (Skill S1 in Skills.Values)
                ...
                {
                    IsEnabled = true,
                    Color = Color.White.ToVector4() * 0.1f,
                    Power = 2f,
                    LightDecay = 150,
        ...
        public void Draw(SpriteBatch spriteBatch, GraphicsDevice graphicsDevice, SkillSelector Selector, Rectangle viewport)
        {
            UpdateCamera(viewport, Selector.cam, Selector);

            Camera cam = Selector.cam;
            
            cam.SetRenderTarget(graphicsDevice, RenderMode.Regular);
            DrawBackground(spriteBatch, graphicsDevice, cam, viewport);
            DrawLines(spriteBatch, graphicsDevice, cam, Selector);
            DrawSkills(spriteBatch, graphicsDevice, cam, Selector);

            cam.SetRenderTarget(graphicsDevice, RenderMode.UI);
            DrawInfo(spriteBatch, graphicsDevice, cam, Selector);

        }

        private void DrawBackground(SpriteBatch spriteBatch, GraphicsDevice graphicsDevice, Camera cam, Rectangle viewport)
        {
            spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.Opaque, SamplerState.PointClamp, null, null);

            //new SkeletonTexture("Core", "Marker").Draw(spriteBatch, (Vector2.Zero) + (new Vector2(viewport.Width, viewport.Height) / 2f)
            //    , new Color(35, 35, 35), 0f, new Vector2(viewport.Width, viewport.Height), SpriteEffects.None);
            new SkeletonTexture("Core", "Marker").Draw(spriteBatch, (Vector2.Zero) + (new Vector2(viewport.Width, viewport.Height) / 2f)
                , Color.Purple, 0f, new Vector2(viewport.Width, viewport.Height), SpriteEffects.None);
            

            //spriteBatch.Draw(SheetManager.GetRenderTexture("Core", "Marker"), new Rectangle(0, 0, (int)(viewport.Width / cam.Zoom), (int)(viewport.Height / cam.Zoom)), new Color(35, 35, 35));
            
            spriteBatch.End();
        }


        private void DrawLines(SpriteBatch spriteBatch, GraphicsDevice graphicsDevice, Camera cam, SkillSelector Selector)
        {
                ...
                foreach (KeyValuePair<string, Skill> S1 in Skills)
                    if (!S1.Value.Hidden || ShowAll)
                        if (S1.Value.Parent != null)
                        {
                            ...
                            Line.DrawLine(spriteBatch, S1.Value.Position, S1.Value.Parent.Position, Color.White, 2.5f);
                        }
            }
            spriteBatch.End();
        }

        private void DrawSkills(SpriteBatch spriteBatch, GraphicsDevice graphicsDevice, Camera cam, SkillSelector Selector)
        {
            cam.SetRenderTarget(graphicsDevice, RenderMode.Regular);
        ...
        private void DrawInfo(SpriteBatch spriteBatch, GraphicsDevice graphicsDevice, Camera cam, SkillSelector Selector)
        {
            spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp, null, null, null, Selector.cam.Transform);
            {
                foreach (KeyValuePair<string, Skill> S1 in Skills)
